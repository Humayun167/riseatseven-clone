...
export default function Navbar() {
  ...
  useEffect(() => {
    const handleScroll = () => {
      ...
      const previousScrollY = handleScroll.previousScrollY || 0;
      ...
      handleScroll.previousScrollY = scrollY;
    };
  ...
  // Function to pin hover bg to a specific item
  const pinHoverBg = (label) => {
    ...
    <>
      ...
      <div
        className={`w-full fixed top-0 left-0 z-50 flex transition-all duration-700 h-18 lg:h-20  ${hideHeader ? "-translate-y-full" : ""
          } ${mobileMenu ? "bg-transparent" : ""} px-2.5 ${isAtTop ? "pt-20" : "pt-2.5"}`}
      ...
      >
        ...
        <div className="relative w-full flex items-center justify-between px-4 lg:px-6 py-3">
          {/* Logo */}
          <Link to="/" className="w-32 inline-flex md:w-40 z-100">
            <svg
                ...
                {item.children && activeSubmenu === item.label && (
                  ...
                  >
                    ...
                    >
                        ...
                        <div className={`grid ${item.children.length < 5 ? "grid-cols-1" : "grid-cols-2"} gap-x-12 gap-y-0.5`}>
                          {item.children.map((child) => (
                            <Link
                              key={child.label}
                              to={child.href}
                              className="group block py-px relative overflow-hidden"
                      ...
                      {/* Right: Image Preview */}
                      <div className="relative w-62.5 h-62.5 rounded-2xl overflow-hidden bg-grey-900 shrink-0 shadow-sm">
                        ...
                        {/* Overlay with Button */}
                        {item.label === "Services" && <div className="absolute inset-0 flex items-end justify-start p-6 bg-linear-to-t from-black/40 to-transparent">
                          <a
                            href={item.href}
        ...
        >
          ...
          >
            ...
            <div className="flex items-center justify-between mb-10">
              <Link to="/" className="w-32" onClick={closeMobileMenu}>
                <svg
