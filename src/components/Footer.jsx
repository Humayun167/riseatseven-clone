export default function Footer() {
  ...
  return (
    <footer className="bg-[#111212] text-white p-3 lg:p-6 mt-2 rounded-3xl mx-3 mb-3 overflow-hidden">
      <div className="max-w-[1600px] mx-auto pt-5">
        {/* Top Grid */}
        <div className="grid grid-cols-2 lg:grid-cols-12 gap-x-4 lg:gap-x-0 gap-y-8 lg:gap-y-0 mb-12">
          {/* Column 1 - Newsletter & Socials */}
          <div className="col-span-2 lg:col-span-6 pr-0 lg:pr-20">
            <h3 className="text-lg lg:text-2xl font-medium mb-2">Stay Updated with Rise News</h3>
            ...
            <div className="relative mb-3 max-w-md">
              <input
                ...
                className="w-full rounded-full bg-white/10 px-6 py-2.5 pr-12 focus:outline-none focus:border-white/20 focus:border-2 transition-colors text-sm"
              />
              <button className="absolute right-1.5 top-1/2 -translate-y-1/2 w-9 h-9 bg-mint rounded-full flex items-center justify-center group hover:bg-white hover:text-grey-900 transition-transform cursor-pointer">
                <i className="fa-solid fa-arrow-right text-grey-900 text-xs -rotate-45 group-hover:rotate-45 transition-transform" />
              </button>
            </div>
            ...
            <div className="flex flex-wrap gap-1">
              {socials.map((social) => (
                <a
                  key={social.name}
                  href="#"
                  className="flex items-center gap-1.5 px-1.5 py-0.5 bg-white text-grey-900 rounded-full hover:rounded-md transition-transform duration-500 ease-in-out group text-xs"
                >
                  <i className={`fa-brands ${social.icon} text-[10px]`} />
                  <i className="fa-solid fa-arrow-up-right opacity-0 group-hover:opacity-100 transition-opacity text-[10px]" />
                </a>
              ))}
            </div>
          </div>
          ...
          {menus.map((menu, index) => (
            <div
              key={menu.title}
              className={`col-span-1 md:col-span-1 lg:col-span-2 pl-3 border-l border-grey-600`}
            >
              <h4 className="text-sm font-semibold mb-2">{menu.title}</h4>
              <div className="flex flex-col gap-0.5">
                {menu.items.map((item) => (
                  <a
                    key={item}
                    href="#"
                    className="text-base md:text-lg text-white font-medium relative w-fit overflow-hidden group py-0.5 text-sm"
                  >
        ...
        <div className="w-full lg:pt-6">
          <div className="w-full inline-flex z-50 max-h-12">
            <svg
        ...
        {/* Bottom Bar */}
        <div className="flex flex-col lg:flex-row gap-1 justify-between pt-5">
